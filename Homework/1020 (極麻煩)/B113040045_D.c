...
double fun_lf(double A,double B,double C,double D,double x){
	/* 計算方程式的值 */
	double y=0;
	y = A*pow(x,3)+B*pow(x,2)+C*x+D;
...
void create_line_lf(double x1,double y1,double x2,double y2,double *q_lf,double *r_lf){
	//y=q*x+r , 以指標h回傳q、r的值 
	*q_lf = ( (double)y1-(double)y2 ) / ( (double)x1-(double)x2 );
	
	*r_lf = (double)y1 - (*q_lf) * (double)x1;
}
...
int main(){
	...
	do{
		x2 = (x2+x1)/2;
		close_times++;
	}while(close_times<=3 || fabs(x2-x1) >= 0.1);//fabs取絕對值 
	
	y1 = fun_lf(a,b,c,d,x1);
	y2 = fun_lf(a,b,c,d,x2);
	
	create_line_lf(x1,y1,x2,y2,&q_lf,&r_lf);
	...
	*/
		...
		/* 切線 */
		for(int j=-30;j<=30;j++){
			square[ limit_y - ( calculate_line(q_lf,r_lf,j) +30 ) ][2*(j+30)]=7;
		}
		
		/* 切點 */
		square[ limit_y - ( fun(a,b,c,d,x_1)+30 ) ][ 2*(x_1+30) ]=6;
			...
			if(j%5==0){
				...
				for(int k = 2*(j+30) + count ; k < 2*(j+30) + 10 ; k++){
					if(square[i][k]==0) 
						...
						printf("%c",'-');
					else if(square[i][k]==4)
						printf("%c",'+');
					else if(square[i][k]==5)
						printf("%c",'|');
					else if(square[i][k]==7)
						printf("%c",'#');
					else
						printf("%c",'@');
				}
